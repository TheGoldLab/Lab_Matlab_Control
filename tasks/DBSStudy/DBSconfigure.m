...
name = 'KASStudy';
dotsTheMachineConfiguration.setDefaultValue('dataPath','/Users/KAS/Documents/MATLAB/AuditoryTaskDev/TestingData');
% Other defaults
settings = { ...
   'taskSpecs',                  {'Aud' 1 }, ... %'NN' 1 'Quest' 1 'AN' 1 'SN' 1 'NL' 1 'NR' 1}, ...
   'runGUIname',                 'eyeGUI', ...
   'databaseGUIname',            [], ...
   'remoteDrawing',              false, ...
...
for ii = 1:length(readables)
   theHelper = topNode.addReadable('readable', ...
      ...
      topNode.nodeData{'Settings'}{'showEye'}, ...
      readables{ii});
...
for ii = 1:10
   callStrings{ii} = {'string', sprintf('Next task starts in: %d', 10-ii+1), ...
       'y', -6};
end
countdown = {@showMultiple, topNode.helpers.feedback, ...
   'text', callStrings, 'image', {2, 'y', 3, 'height', 13}, ...
...
for ii = 1:2:length(taskSpecs)
   ...
   args = {taskSpecs{ii},                 ...
      ...
      'taskID',                           (ii+1)/2, ...
      'taskTypeID',  find(strcmp(taskSpecs{ii}, {'VGS' 'MGS' 'Quest' 'Aud' 'NL' 'NR' 'SN' 'SL' 'SR' 'AN' 'AL' 'AR'}),1)};
   ...
   switch taskSpecs{ii}
      ...
      otherwise
         ...
         task = topsTreeNodeTaskAudLocal_Dev;% .getStandardConfiguration(args{:});
         task.setIndependentVariableByName('direction', 'value', ...
