function [overtask, list] = caltask(iter, donts)
%% Calibration Task
...
calibList = {'name', 'entry', 'input', 'exit', 'timeout', 'next';
                 'Draw', {@calibpoint list}, {}, {}, 0, 'Show';
                 'Show', {show [targInd cursorInd]}, {}, {}, changetime, 'Change';
                 'Change', {change cursorInd}, {}, {}, 0, 'Feedback';
                 'Feedback', {@feedback list}, {}, {}, 1, 'Hide';
                 'Hide',{hide [targInd cursorInd]}, {}, {}, 0, 'Changeback';
                 'Changeback', {changeback cursorInd}, {}, {}, 0, '';};
...
function calibpoint(list)
    ...
    cursorInd = list{'Graphics'}{'CursorInd'};
    targInd = list{'Graphics'}{'TargetInd'};
    
    %set target and cursor properties for this drawn location
    %Use x/y positions from Calib structure--but adjust to fit Tobii's
    %coordinate system by subtracting 0.5. Tobii uses a coordinate system
    %with zero in the bottom-right corner, while Snowdots draws things with
    %(0,0) in the center. 
    X = 0;%20*(Calib.points.x(morder(counter))-0.5);
    Y = 0;%20*(Calib.points.y(morder(counter))-0.5);
    
%     ensemble.setObjectProperty('xCenter', X, targInd)
%     ensemble.setObjectProperty('yCenter', Y, targInd)
%     
%     ensemble.setObjectProperty('xCenter', X, cursorInd)
%     ensemble.setObjectProperty('yCenter', Y, cursorInd)
...
%     tetio_addCalibPoint(Calib.points.x(morder(counter)),Calib.points.y(morder(counter)));
    fprintf('Drawing point at %d, %d', X, Y);
end
...
function feedback(list)
    ...
    %Import Calib, ensemble and cursorInd
    Calib = list{'Calib'}{'Calib'};
    ensemble = list{'Graphics'}{'Ensemble'};
    cursorInd = list{'Graphics'}{'CursorInd'};
    ...
    while toc < rtwindow
            ...
            if ~isempty(eventname)
                ensemble.setObjectProperty('colors', [0 1 0], cursorInd)
            end
