...
%   the value 1 is ambiguous, ignore it as a CLUT index.
if isscalar(ptrOrLum) && ptrOrLum < 1 && ptrOrLum >=0
    if g.loadGammaBitsPlus
        ...
        bglum16bit = g.gamma16bit(1+round(ptrOrLum*(2^16-1)));
        rgb_(3) = 0;
        rgb_(2) = uint8(mod(bglum16bit, 256));
        rgb_(1) = uint8(floor(bglum16bit/256));
    else
