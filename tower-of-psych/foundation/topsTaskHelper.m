   ...
   methods
      ...
      function [theObject, indices] = parse(self, name, theObject, fevalable, ...
         ...
         elseif ~isempty(unmatched)
            ...
            % Collect objects
            objects = cell(numObjects, 2);
            indices = struct();
            isDrawable = false;
            for ii = 1:numObjects
               
               % get arguments
               if isfield(unmatched.(names{ii}), 'fevalable')
                  objectFevalable = unmatched.(names{ii}).fevalable;
               else
                  objectFevalable = names{ii};
               end
               if isfield(unmatched.(names{ii}), 'settings')
                  objectSettings = unmatched.(names{ii}).settings;
               else
                  objectSettings = struct();
               end
               
               % Recursively parse
               [objects{ii,1}, objects{ii,2}] = self.parse(names{ii}, [], ...
                  objectFevalable, objectSettings, commonSettings, []);
               
               % Update the indices
               if isstruct(objects{ii,2})
                  indices.(names{ii}) = objects{ii,2};
               else
                  indices.(names{ii}) = ii;
               end
               
               % Check for drawable
               if isa(objects{ii}, 'dotsDrawable')
                  isDrawable = true;
               end
            end
            ...
            elseif isDrawable
               ...
               theObject = dotsDrawable.makeEnsemble(name, objects, true);
