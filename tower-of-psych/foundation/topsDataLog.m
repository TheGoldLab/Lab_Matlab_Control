      ...
      function ecodes = parseEcodes(group, fileWithPath)
         ...
         if ~isempty(logStruct)
            ...
            for ii = sort(igroups)'
               ...
               cellData = permute(struct2cell(...
                  [logStruct(inds).item]),[3 1 2]);
               
               % Convert empties to nans
               Lempty = cellfun('isempty', cellData);
               cellData(Lempty) = {nan};
               
               % Make sure everything is a double and convert to matrix
               grdata = cellfun(@double, cellData);
