function demoStateMachineStimulus
% function demoStateMachineStimulus
% 
...
%% Open a snowdot window
displayInfo = mglDescribeDisplays();

% dotsTheScreen wants distance values in cm. We convert accordingly.
sc = dotsTheScreen.theObject();
sc.distance = 60;
% sc.width  = displayInfo(2).screenSizeMM(1)/10;
% sc.height = displayInfo(2).screenSizeMM(2)/10;
sc.displayIndex = 0;
% sc.windowRect = sc.displayPixels;
sc.initialize();
sc.openWindow();
...
state{'stimulus'}{'trialsPerCoherencePerDirection'} = 5;
...
test = topsEnsemble();
test.addObject(maintask);
test.callObjectMethod(@run);
