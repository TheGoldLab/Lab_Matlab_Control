...
% learning.string = strcat(num2str(BucketPosition),'=',num2str(PreviousBucketPosition),'+', num2str(delta),'-(',num2str(PreviousGB),'-',num2str(PreviousBucketPosition),')');
...
trialCalls.addCall({@update, list, compMouse, MouseDot, starget, stext, htarget, htext, rtarget, rtext, ptarget, fixedMachine}, 'update');
 
 fixedStates = { ...
    'name'      'entry'    'next' 'timeout' 'exit'  'input'; ...
    'position'  {@startTrial, starget, htarget, rtarget}           'set'     0         {}  {}; ...
    'set'       {@Set_cue, list, gauss} 'animate'       0        {}                  {};...   
    'animate'   {@startAnimation, list, arrow_animated, target_animated, learning, ballAnimator_samp}      ''       2      {} {}; ... 
     };
...
% FUNCTION: startTrial
%
function startTrial(starget, htarget, rtarget) 
global counter
counter = counter + 1
...
list{'Stimulus'}{'GreenMeanlist'} = [list{'Stimulus'}{'GreenMeanlist'}, distribS];
...
% BucketPosition =  PreviousBucketPosition + delta - PredictionError
BucketPosition =  PreviousBucketPosition + delta - previousGreen(counter) - PreviousBucketPosition;

target_animated.addMember('x', [0 .5], [PreviousBucketPosition BucketPosition], true);
learning.string = strcat(num2str(BucketPosition),'=',num2str(PreviousBucketPosition),'+', num2str(delta),'-',num2str(previousGreen(counter)),'-',num2str(PreviousBucketPosition));
...
list{'learning'}{'PreviousBucketPosition'} = BucketPosition;
% +++++++++
% FUNCTION: Update mouse
%

function update(list, compMouse, MouseDot, starget, stext, htarget, htext, rtarget, rtext, ptarget, fixedMachine)
