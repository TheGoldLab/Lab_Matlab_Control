...
%
% Ben Heasly modified the task in 2011 to work with October2011 version of
% Snow Dots

clear all
clear classes

% Keep track of time using mgl, this affects timestamps in DATA-file
d=topsDataLog.theDataLog;
d.clockFunction = @mglGetSecs;
...
% control variables
taskList{'control'}{'remote'} = 0;
taskList{'control'}{'fixed outcomes'} = 0;     % if fixed outcomes == 1, outcomes are drawn sequentially from fixed outcome array
...
taskList{'control'}{'seed'} = TIME(6)*10e6;    % reset random seed (to clock TIME(6)*10e6 or to specified value)
...
taskList{'control'}{'trials per block'} = 10; % how many trials per block?
...
taskList{'control'}{'session time'} = sprintf('%d', TIME(1:5));
taskList{'control'}{'alpha reset control'} =0;

% instruction variables
taskList{'instructions'}{'instructions'} = 0 ;        % show instructions before starting task?
% number of seconds to show each slide... can take array of length = instruction slides
taskList{'instructions'}{'seconds per slide'} = [7 15 15 15 7];
fileNames = { ...
    'Titan_Attacks1.tiff', ...
    'Titan_Attacks2.tiff', ...
    'Titan_Attacks3.tiff', ...
    'Titan_Attacks4.tiff', ...
    'Titan_Attacks5.tiff'};
taskList{'instructions'}{'file names'} = fileNames;

% feedback variables
taskList{'feedback'}{'feedback on'} = 0;                     %Turns on feedback at the end of each block
...
taskList{'feedback'}{'font size'}=30;
...
taskList{'graphics'}{'fixation size'} = 1;            % size of fixation point in degrees
...
taskList{'graphics'}{'font size'} = 48;                 % how big?
taskList{'graphics'}{'outcome text Y'} = 1.25;             % Y-axis value for center of outcome number
taskList{'graphics'}{'prediction text Y'} = -1.25;         % Y-axis value for center of prediction number
taskList{'graphics'}{'disappear'} = 1;                   % 1 = subjects only see outcome for 2 seconds.

% line graphics
taskList{'graphics'}{'lines on'} = 1;                   % do you want to show a spatial (line) representation?
if taskList{'screen'}{'full screen'}
    taskList{'graphics'}{'screen scale'} = 30;              % scales width of total spatial representation
else
    taskList{'graphics'}{'screen scale'} = 15;              % scales width of total spatial representation
end
...
if taskList{'sounds'}{'switch sounds'}
    ...
    taskList{'sounds'}{'sound list'} = getPredInfSounds();
...
dm.waitFevalable={@mglWaitSecs, .0001};
...
fp.size = taskList{'graphics'}{'fixation size'};
fp.isVisible = taskList{'graphics'}{'isoluminance'};
...
prediction.xFrom = taskList{'graphics'}{'prediction start X'};
prediction.xTo = taskList{'graphics'}{'prediction start X'};
prediction.yFrom = taskList{'graphics'}{'prediction Y'} + taskList{'graphics'}{'line length'};
prediction.yTo = taskList{'graphics'}{'prediction Y'};
...
outcome.xFrom = -10e20;
outcome.xTo = -10e20; % initial position= off screen
outcome.yFrom = taskList{'graphics'}{'outcome Y'}+taskList{'graphics'}{'line length'};
outcome.yTo = taskList{'graphics'}{'outcome Y'};
...
errorL.xFrom = -20;
errorL.xTo = -20;
errorL.yFrom = taskList{'graphics'}{'error Y'};
errorL.yTo = taskList{'graphics'}{'error Y'};
...
boundL1.xFrom = -.5*taskList{'graphics'}{'screen scale'};
boundL1.xTo = .5*taskList{'graphics'}{'screen scale'};
boundL1.yFrom = taskList{'graphics'}{'boundary line 1 Y'};
boundL1.yTo = taskList{'graphics'}{'boundary line 1 Y'};
...
boundL2.xFrom = -.5*taskList{'graphics'}{'screen scale'};
boundL2.xTo = .5*taskList{'graphics'}{'screen scale'};
boundL2.yFrom = taskList{'graphics'}{'boundary line 2 Y'};
boundL2.yTo = taskList{'graphics'}{'boundary line 2 Y'};
...
outcomeText.fontSize = taskList{'graphics'}{'font size'};
...
predictionText.fontSize = taskList{'graphics'}{'font size'};
...
% Instruction images
instruct= dm.newObject('dotsDrawableImages');
instruct.fileNames = taskList{'instructions'}{'file names'};
taskList{'graphics'}{'instructions images'}=instruct;
...
if taskList{'feedback'}{'feedback on'}
    ...
    feedBackText.fontSize = taskList{'feedback'}{'font size'};
    feedBackText.x = 0;
    feedBackText.y = 0;
...
% GOOD = Wait until you get an open window... open drawables window... open
% a window then close it... then get windowRect for that pix box... that
% should tell you the numbher of pixels on the remote computer screen.
...
%       - topsCallList organizes moment-to-moment, concurrent behaviors
%       within trials
...
dataPath = dotsTheSwitchboard.getSharedPropertyValue('dataPath');
dataName = sprintf('%s%s%s', ...
    taskList{'control'}{'data file name'}, ...
    taskList{'control'}{'session time'}, ...
    'taskList.mat');
save(fullfile(dataPath, dataName), 'taskList');
...
mglWaitSecs(taskList{'timing'}{'intertrial'});
...
predictionLine = taskList{'graphics'}{'prediction line'};
pos = feval(@positionFromValue, taskList{'control'}{'prediction'}, taskList);
predictionLine.xFrom = pos;
predictionLine.xTo = pos;
mglWaitSecs(taskList{'timing'}{'fine adjust delay'});
...
outcomeLine=taskList{'graphics'}{'outcome line'};
outcomeLine.xFrom = pos;
outcomeLine.xTo = pos;
outText=taskList{'graphics'}{'outcome text'};
outText.string=sprintf('%d', taskList{'control'}{'outcome'});
errorLine=taskList{'graphics'}{'error line'};
predictionLine=taskList{'graphics'}{'prediction line'};
errorLine.xFrom = outcomeLine.xFrom;
errorLine.xTo = predictionLine.xFrom;
...
screenScale = taskList{'graphics'}{'screen scale'};
maxOutcome = taskList{'control'}{'max outcome'};
pos = value/maxOutcome.*screenScale - .5*screenScale;

%%
function resetTaskList(taskList)
% ask the task tree what block this is
...
dm = dotsTheDrawablesManager.theObject;
if taskList{'feedback'}{'feedback on'}
    % display feedback text about error
    avErr = taskList{'feedback'}{'total error'} ...
        ./ (taskList{'control'}{'trials complete'} ...
        * taskList{'control'}{'block'});
    
    if taskList{'feedback'}{'performance payout'}
        avOmnErr = taskList{'feedback'}{'total omnicient error'} ...
            ./ (taskList{'control'}{'trials complete'} ...
            .* taskList{'control'}{'block'});
        avAmnErr = taskList{'feedback'}{'total amnesic error'} ...
            ./ (taskList{'control'}{'trials complete'} ...
            .* taskList{'control'}{'block'});
        ...
        payoutText.string = sprintf('gold ($15) < %.1f, silver ($12) < %.1f, bronze ($10) < %.1f', ...
            gold, silver, bronze);
...
if taskList{'instructions'}{'instructions'} == 1
    ...
    instruct=taskList{'graphics'}{'instructions images'};
    nSlides = numel(instruct.fileNames);
    secsPerSlide = taskList{'instructions'}{'seconds per slide'};
    
    show(instruct);
    for ii = 1:nSlides
        instruct.slideNumber = ii;
        dm.mayDrawNextFrame;
        
        if numel(secsPerSlide) == nSlides
            mglWaitSecs(secsPerSlide(ii));
        else
            mglWaitSecs(secsPerSlide(1));
        end
    end
    hide(instruct);
...
if strcmp(soundName, 'fixation end') && taskList{'graphics'}{'disappear'}
