...
quest_values = load('scriptRunValues/quest_custom_values.mat');
X = quest_values.X;
threshold_true = quest_values.threshold_true;
gamma = quest_values.gamma;
beta = quest_values.beta;
epsilon = quest_values.epsilon;
delta = quest_values.delta;
grain = quest_values.grain;
Y_true = Weibull(X, threshold_true, gamma, beta, epsilon, delta,grain);

mu = quest_values.mu;
sigma = quest_values.sigma;
Y_pdf = log(normpdf(X,mu,sigma));
...
list{'quest'}{'X'} = X;
list{'quest'}{'gamma'} = gamma;
list{'quest'}{'beta'} = beta;
list{'quest'}{'epsilon'} = epsilon;
list{'quest'}{'delta'} = delta;
list{'quest'}{'grain'} = grain;

list{'quest'}{'Y_true'} = Y_true;
list{'quest'}{'Y_pdf'} = Y_pdf;
...
%Use for actual observer (one more associated below)
...
peak_X = logic.coherence;
threshold = list{'quest'}{'X'};
gamma = list{'quest'}{'gamma'};
beta = list{'quest'}{'beta'};
epsilon = list{'quest'}{'epsilon'};
delta = list{'quest'}{'delta'};
grain = list{'quest'}{'grain'};

Y_S = Weibull(peak_X, threshold, gamma, beta, epsilon, delta, grain);
Y_N = log(1- Y_S)*100;
Y_S = log(Y_S)*100;

%find chance of success at choosen T_post and calculate outcome
Y_true = list{'quest'}{'Y_true'};
Y_pdf = list{'quest'}{'Y_pdf'};

% use for simulated observer
% trial_chance = Y_true(threshold==peak_X);
% trial_result = trial_chance > rand(1);
% if (trial_result)
%     Y_pdf = Y_pdf + Y_S;
%     drawables.setObjectProperty( ...
%       'colors', list{'graphics'}{'green'}, targsInd);
...
peak = max(Y_pdf);
coh = threshold(Y_pdf == peak);
plot(threshold, Y_pdf);
disp(coh);
if coh>100
    coh=100;
end
