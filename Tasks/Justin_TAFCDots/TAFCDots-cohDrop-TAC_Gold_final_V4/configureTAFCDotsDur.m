...
static_dot_reset = ch_values.static_dot_reset;
...
list{'graphics'}{'static_dot_reset'} = static_dot_reset;

%% Set Trial TAC (Unactivated)
...
if (stim.last_changepoint_index - 23 ) == stim.tind;
...
if (stim.last_changepoint_index == stim.tind+1)
    fpObject = drawables.getObject(fpInd);
    fpObject.isVisible = false;
end
...
static_dot_reset = list{'graphics'}{'static_dot_reset'};
...
drawables.setObjectProperty( ...
    'last_changepoint_index', 0, [stimInd]);
drawables.setObjectProperty( ...
    'last_changepoint_reached', false, [stimInd]);
drawables.setObjectProperty( ...
'static_dot_reset', static_dot_reset, [stimInd]);
