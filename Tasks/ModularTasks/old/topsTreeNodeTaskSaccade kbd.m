classdef topsTreeNodeTaskSaccade < topsTreeNodeTask
   ...
   properties
      ...
      readables = struct( ...
         ...
         'choseTarget', 'calibrate'}, ...
         'uiType', ...
         {'default', 'default', 'defaut', 'default', 'dotsReadableHIDKeyboard', ...
         'dotsReadableHIDKeyboard', 'dotsReadableEye'}));
   ...
   methods
      ...
      function setSaccadeChoice(self, value)
         ...
         % --- Save status string
         self.showStatus(sprintf('  %s, RT=%.2f (mean RT=%.2f)', ...
            feedbackString, trial.RT, nanmean([self.trialData.RT])));
      end
   end
   ...
   methods (Access = private)
      ...
      function prepareReadables(self)
         ...
         % Always update the target gaze window
         self.readables.userInput.defineCompoundEvent('trgWindow', 'centerXY', [...
            self.drawables.stimulusEnsemble.getObjectProperty('xCenter', 2), ...
            self.drawables.stimulusEnsemble.getObjectProperty('yCenter', 2)]);
      ...
      function initializeTrialData(self)
         ...
         self.trialData = dealMat2Struct(self.trialData(1), ...
            'taskID', repmat(self.taskID,1,numel(directions)), ...
