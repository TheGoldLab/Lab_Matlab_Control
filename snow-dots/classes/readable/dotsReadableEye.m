classdef dotsReadableEye < dotsReadable
    ...
    properties
        ...
        % how many samples to average for "current x/y" above
        %   jig -- could change this from samples to time
        gazeBufferLength = 10;
    ...
    properties (SetAccess = protected)
        ...
        % Store rotating array of gaze samples to compute mean
        gazeBuffer = [];
    end
    ...
    methods
        ...
        function addGazeWindow(self, gazeWindowName, varargin)
            ...
            % Possibly set to mean gaze position from buffer
            if self.gazeEvents(index).setToGaze
                self.translation(1) = self.translation(1) + self.x - ...
                    self.gazeEvents(index).centerXY(1);
                self.translation(2) = self.translation(2) + self.y - ...
                    self.gazeEvents(index).centerXY(2);
                self.gazeEvents(index).setToGaze = false;
            end
        ...
        function calibrateDevice(self)
            ...
            for ii = 1:numFixations
                ...
                for jj = 1:self.calibrationN
                    ...
                    data(jj,:) = dataMatrix([self.xID, self.yID], 2)';
                end
            ...
            % Possibly initialize the gaze buffer [time x y]
            if isempty(self.gazeBuffer)
                self.gazeBuffer = nans(self.gazeBufferLength, 3);
            end
        end
    ...
    methods (Static)
        ...
        % setGazeWindows(ui)
        %
        % Convenient utility for checking that the readable is a
        % dotsReadableEye and then, if so, setting a bunch of gaze windows
        %
        % Arguments are cell arrays of arguments to
        %   dotsReadableEye.addGazeWindow
        %   (i.e., property/value pairs)
        function setGazeWindows(ui, varargin)
