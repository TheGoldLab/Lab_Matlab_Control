...
%          dataMatrix = cell2num(dataMatrix);
   ...
   methods (Access = protected)
      ...
      function newData = readRawEyeData(self)
         ...
         if sum(LeyeH) == sum(LeyeV) + 1
            LeyeH(1) = false;
         elseif sum(LeyeV) == sum(LeyeH) + 1
            LeyeV(1) = false;
         elseif sum(LeyeV) ~= sum(LeyeH)
            disp('here')
         end
         ...
         %  <ID> <value> <timestamp>
         newData = cat(1, ...
            [repmat(self.xID, sum(LeyeH), 1) v(LeyeH)'.*30 t(LeyeH)'], ...
            [repmat(self.yID, sum(LeyeV), 1) v(LeyeV)'.*30 t(LeyeV)']);
