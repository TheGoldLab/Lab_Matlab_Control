classdef dotsReadableHID < dotsReadable
   ...
   methods
      ...
      function components = setComponentCalibration(self, namesOrIDs, ...
            rawRange, deadRange, calibratedRange, granularity)
         
         % find components indexes for each ID
         if isnumeric(namesOrIDs)
            compIndexes = self.getComponentIndicesByID(namesOrIDs);
         else
            compIndexes = self.getComponentIndicesByName(namesOrIDs);
         end
      ...
      % @param componentNameOrID the HID usage name or ID 
      % @details
      % Finds the component that matches the given @a nameOrID, and
      % defines an event for that component.  Any existing event
