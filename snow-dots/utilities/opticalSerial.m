classdef opticalSerial < handle
   ...
   %   1 stop bit
   %
   %   If you get an error because the port is still open, try:
   %   a = instrfind;
   %   fclose(a(**)); % ** is the index of the port you want to close
   %
   ...
   methods
      ...
      function OP = opticalSerial(portName)
         ...
         % Check for default
         if isempty(OP.portName)
            calDir = dir(sprintf('/dev/%s*', OP.defaultName));
            if ~isempty(calDir)
               OP.portName = ['/dev/' calDir(1).name];
            else
               error('opticalSerial: port not found')
            end
         end
